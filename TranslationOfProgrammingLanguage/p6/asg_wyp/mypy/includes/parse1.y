...
%type<node> expr_stmt small_stmt print_stmt opt_test

%type<node> test testlist star_COMMA_test exprlist
%type<node> atom power factor term arith_expr shift_expr
%type<node> and_expr xor_expr expr comparison not_test
%type<node> and_test or_test opt_IF_ELSE
%type<node> opt_yield_test pick_yield_expr_testlist_comp
%type<node> yield_expr testlist_comp pick_yield_expr_testlist
%type<node> star_SEMI_small_stmt simple_stmt stmt compound_stmt
%type<node> plus_stmt suite pick_NEWLINE_stmt star_NEWLINE_stmt
%type<node> funcdef file_input classdef flow_stmt

%type<intNumber> pick_unop pick_multop pick_PLUS_MINUS augassign
%type<intNumber> comp_op pick_LEFTSHIFT_RIGHTSHIFT star_trailer trailer

%token<intNumber> MINUS PLUS TILDE STAR SLASH PERCENT DOUBLESLASH
%token<intNumber> LEFTSHIFT RIGHTSHIFT

%token<intNumber> PLUSEQUAL MINEQUAL STAREQUAL SLASHEQUAL PERCENTEQUAL
%token<intNumber> AMPEREQUAL GRLT GREATEREQUAL LESSEQUAL NOTEQUAL EQEQUAL
%token<intNumber> LESS GREATER IS IN VBAREQUAL CIRCUMFLEXEQUAL LEFTSHIFTEQUAL
%token<intNumber> RIGHTSHIFTEQUAL DOUBLESTAREQUAL DOUBLESLASHEQUAL
	...
	| stmt
		{
			if ( $1 ) {
				...
				std::cout << "Can't evaluate the thing" << std::endl;
			}
		}
	;
...
power // Used in: factor
	...
	| atom star_trailer
		{
			if(!$2) {
				str::string n = reinterpret_cast<IdentNode*>($1)->getIdent();
				$$ = new CallNode(n);
				pool.add($$);
			}
			else {
