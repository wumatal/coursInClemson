...
void display ( void )   // Create The Display Function
{
	...
	for(int i=0; i<4; i++) {
		...
		else if ( count_points >= MAX_POINTS){
			typeset[1] = 0.101; typeset[1] = 0.31; typeset[1] = 0.337;
			//typeset[0] = 0.008; typeset[1] = 0.078; typeset[2] = 0.478; 
			typeset[3] = 7.7;
			write_pixel(ctrl_points[i][0], ctrl_points[i][1], typeset);
		}
	}
...
void mouse(int button, int state, int x, int y)
{
	...
	while ( index != INVALID && button != GLUT_LEFT_BUTTON) {

		// Maintain the old point.
		typeset[0] = 0.008; typeset[1] = 0.078; typeset[2] = 0.478; 
		typeset[3] = 7.7;
		write_pixel(x, y, typeset);		
	}
